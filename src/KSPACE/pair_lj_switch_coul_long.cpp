...
#include "pair_lj_switch_coul_long.h"
#include <cmath>
#include <cstring>
#include "atom.h"
#include "comm.h"
...
PairLJSwitchCoulLong::PairLJSwitchCoulLong(LAMMPS *lmp) : Pair(lmp)
{
  respa_enable = 1;
...
PairLJSwitchCoulLong::~PairLJSwitchCoulLong()
{
  if (copymode) return;
...
void PairLJSwitchCoulLong::compute(int eflag, int vflag)
{
...
void PairLJSwitchCoulLong::compute_inner()
{
...
void PairLJSwitchCoulLong::compute_middle()
{
  int i,j,ii,jj,inum,jnum,itype,jtype;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,fpair;
  double rsq,r2inv,r6inv,forcecoul,forcelj,factor_coul,factor_lj;
...
void PairLJSwitchCoulLong::compute_outer(int eflag, int vflag)
{
  int i,j,ii,jj,inum,jnum,itype,jtype,itable;
  double qtmp,xtmp,ytmp,ztmp,delx,dely,delz,evdwl,ecoul,fpair;
  double fraction,table;
...
void PairLJSwitchCoulLong::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairLJSwitchCoulLong::settings(int narg, char **arg)
{
...
void PairLJSwitchCoulLong::coeff(int narg, char **arg)
{
...
void PairLJSwitchCoulLong::init_style()
{
  if (!atom->q_flag)
    error->all(FLERR,
...
double PairLJSwitchCoulLong::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairLJSwitchCoulLong::write_restart(FILE *fp)
{
...
void PairLJSwitchCoulLong::read_restart(FILE *fp)
{
...
void PairLJSwitchCoulLong::write_restart_settings(FILE *fp)
{
...
void PairLJSwitchCoulLong::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJSwitchCoulLong::write_data(FILE *fp)
{
...
void PairLJSwitchCoulLong::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
...
double PairLJSwitchCoulLong::single(int i, int j, int itype, int jtype,
                                    double rsq,
...
void *PairLJSwitchCoulLong::extract(const char *str, int &dim)
{
  dim = 2;
