...
#include <cstring>
#include <cctype>
#include "pair_hybrid_coupling.h"
#include "atom.h"
#include "force.h"
#include "pair.h"
#include "neighbor.h"
...
PairHybridCoupling::PairHybridCoupling(LAMMPS *lmp) : Pair(lmp),
...
PairHybridCoupling::~PairHybridCoupling()
{
  if (nstyles > 0) {
...
void PairHybridCoupling::compute(int eflag, int vflag)
{
  int i,j,m,n;
  ...
  for (m = 0; m < nstyles; m++) {
    ...
    if (eflag_global) {
      ...
      eng_coul += factor*styles[m]->eng_coul;
    }
    if (vflag_global) {
      for (n = 0; n < 6; n++) virial[n] += factor*styles[m]->virial[n];
    }
    if (eflag_atom) {
      double *eatom_substyle = styles[m]->eatom;
      for (i = 0; i < ntotal; i++) eatom[i] += factor*eatom_substyle[i];
    }
    if (vflag_atom) {
      double **vatom_substyle = styles[m]->vatom;
      for (i = 0; i < ntotal; i++)
        for (j = 0; j < 6; j++)
          vatom[i][j] += factor*vatom_substyle[i][j];
    }
  }
...
void PairHybridCoupling::add_tally_callback(Compute *ptr)
{
  for (int m = 0; m < nstyles; m++)
...
void PairHybridCoupling::del_tally_callback(Compute *ptr)
{
  for (int m = 0; m < nstyles; m++)
...
void PairHybridCoupling::compute_outer(int eflag, int vflag)
{
  outerflag = 1;
  compute(eflag,vflag);
  outerflag = 0;
...
void PairHybridCoupling::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairHybridCoupling::coeff(int narg, char **arg)
{
...
void PairHybridCoupling::init_style()
{
...
double PairHybridCoupling::init_one(int i, int j)
{
...
void PairHybridCoupling::write_restart(FILE *fp)
{
...
void PairHybridCoupling::read_restart(FILE *fp)
{
  int me = comm->me;
...
double PairHybridCoupling::single(int i, int j, int itype, int jtype,
...
void PairHybridCoupling::modify_params(int narg, char **arg)
{
...
void PairHybridCoupling::modify_special(int m, int /*narg*/, char **arg)
{
  double special[4];
  int i;
...
void PairHybridCoupling::set_special(int m)
{
  int i;
  if (special_lj[m])
...
double * PairHybridCoupling::save_special()
{
...
void PairHybridCoupling::restore_special(double *saved)
{
  for (int i = 0; i < 4; ++i) {
...
void *PairHybridCoupling::extract(const char *str, int &dim)
{
  void *cutptr = NULL;
  void *ptr;
  double cutvalue = 0.0;
...
void PairHybridCoupling::reset_dt()
{
...
int PairHybridCoupling::check_ijtype(int itype, int jtype, char *substyle)
{
  for (int m = 0; m < nmap[itype][jtype]; m++)
...
double PairHybridCoupling::memory_usage()
{
