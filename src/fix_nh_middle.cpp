...
void FixNHMiddle::initial_integrate_respa(int /*vflag*/, int ilevel, int /*iloop*/)
{
...
void FixNHMiddle::final_integrate_respa(int ilevel, int /*iloop*/)
{
  // set timesteps by level
...
int FixNHMiddle::size_restart_global()
{
  int nsize = 2;
...
int FixNHMiddle::pack_restart_data(double *list)
{
  int n = 0;
...
int FixNHMiddle::modify_param(int narg, char **arg)
{
...
double FixNHMiddle::compute_scalar()
{
  int i;
  double volume;
  double energy;
...
double FixNHMiddle::compute_vector(int n)
{
  int ilen;
...
void FixNHMiddle::reset_target(double t_new)
{
...
void *FixNHMiddle::extract(const char *str, int &dim)
{
  dim=0;
...
void FixNHMiddle::nhc_temp_integrate()
{
  int ich;
  double expfac;
  ...
  for (int iloop = 0; iloop < nc_tchain; iloop++) {
    ...
    for (ich = mtchain-1; ich > 0; ich--) {
      expfac = exp(-ncfac*dt4*eta_dot[ich+1]);
      eta_dot[ich] *= expfac;
      eta_dot[ich] += eta_dotdot[ich] * ncfac*dthalf;
      eta_dot[ich] *= tdrag_factor;
      eta_dot[ich] *= expfac;
    }

    expfac = exp(-ncfac*dt4*eta_dot[1]);
    eta_dot[0] *= expfac;
    eta_dot[0] += eta_dotdot[0] * ncfac*dthalf;
    eta_dot[0] *= tdrag_factor;
    eta_dot[0] *= expfac;

    factor_eta = exp(-ncfac*dtv*eta_dot[0]);
    nh_v_temp();
...
void FixNHMiddle::nhc_press_integrate()
{
  int ich,i,pdof;
...
void FixNHMiddle::compute_sigma()
{
...
double FixNHMiddle::compute_strain_energy()
{
...
void FixNHMiddle::compute_deviatoric()
{
...
void FixNHMiddle::compute_temp_target()
{
...
void FixNHMiddle::compute_press_target()
{
  double delta = update->ntimestep - update->beginstep;
  if (delta != 0.0) delta /= update->endstep - update->beginstep;
...
double FixNHMiddle::memory_usage()
{
  double bytes = 0.0;
