...
#include "fix_nh_massive_atomic.h"
#include <cstring>
#include <cmath>
#include "atom.h"
#include "force.h"
...
FixNHMassiveAtomic::FixNHMassiveAtomic(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  rfix(nullptr), id_dilate(nullptr), irregular(nullptr),
  id_temp(nullptr), id_press(nullptr),
  eta(nullptr), eta_dot(nullptr),
...
FixNHMassiveAtomic::~FixNHMassiveAtomic()
{
  if (copymode) return;
...
int FixNHMassiveAtomic::setmask()
{
  int mask = 0;
...
void FixNHMassiveAtomic::setup(int /*vflag*/)
{
...
void FixNHMassiveAtomic::initial_integrate(int /*vflag*/)
{
...
void FixNHMassiveAtomic::final_integrate()
{
  nve_v();
...
void FixNHMassiveAtomic::initial_integrate_respa(int /*vflag*/, int ilevel, int /*iloop*/)
{
...
void FixNHMassiveAtomic::final_integrate_respa(int ilevel, int /*iloop*/)
{
  // set timesteps by level
...
void FixNHMassiveAtomic::couple()
{
...
void FixNHMassiveAtomic::write_restart(FILE *fp)
{
...
int FixNHMassiveAtomic::size_restart_global()
{
  int nsize = 2;
...
int FixNHMassiveAtomic::pack_restart_data(double *list)
{
  int n = 0;
...
void FixNHMassiveAtomic::restart(char *buf)
{
  int n = 0;
...
int FixNHMassiveAtomic::modify_param(int narg, char **arg)
{
...
double FixNHMassiveAtomic::compute_scalar()
{
  int i;
  double volume;
  double energy;
...
double FixNHMassiveAtomic::compute_vector(int n)
{
  int ilen;
...
void FixNHMassiveAtomic::reset_target(double t_new)
{
...
void FixNHMassiveAtomic::reset_dt()
{
  dtv = update->dt;
  dtf = 0.5 * update->dt * force->ftm2v;
  dthalf = 0.5 * update->dt;
...
void *FixNHMassiveAtomic::extract(const char *str, int &dim)
{
  dim=0;
...
void FixNHMassiveAtomic::nhc_temp_integrate()
{
  int ich;
  double expfac;
  double kt = boltz * t_target;
...
void FixNHMassiveAtomic::nhc_press_integrate()
{
  int ich,i,pdof;
...
void FixNHMassiveAtomic::nh_v_press()
{
  double factor[3];
  double **v = atom->v;
  int *mask = atom->mask;
...
void FixNHMassiveAtomic::nh_v_temp()
{
  double **v = atom->v;
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixNHMassiveAtomic::compute_sigma()
{
...
double FixNHMassiveAtomic::compute_strain_energy()
{
...
void FixNHMassiveAtomic::compute_deviatoric()
{
...
void FixNHMassiveAtomic::compute_temp_target()
{
...
void FixNHMassiveAtomic::compute_press_target()
{
  double delta = update->ntimestep - update->beginstep;
  if (delta != 0.0) delta /= update->endstep - update->beginstep;
...
void FixNHMassiveAtomic::nh_omega_dot()
{
...
void FixNHMassiveAtomic::pre_exchange()
{
...
double FixNHMassiveAtomic::memory_usage()
{
...
void FixNHMassiveAtomic::grow_arrays(int nmax)
{
  memory->grow(eta, nmax, 3, mtchain, "fix_nh_massive_atomic:eta");
  memory->grow(eta_dot, nmax, 3, mtchain+1, "fix_nh_massive_atomic:eta_dot");
}
...
void FixNHMassiveAtomic::copy_arrays(int i, int j, int /*delflag*/)
{
...
int FixNHMassiveAtomic::pack_exchange(int i, double *buf)
{
...
int FixNHMassiveAtomic::unpack_exchange(int nlocal, double *buf)
{
