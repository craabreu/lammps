...
double FixNHMassive::compute_scalar()
{
  int i;
  double volume;
  double energy;
...
double FixNHMassive::compute_vector(int n)
{
  int ilen;
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      for (int iloop = 0; iloop < nc_tchain; iloop++) {
        ...
        v[i][2] *= exp(-dt2loop*v_eta[i][2]);

        if (deterministic_flag) {
          eta[i][0] += dt2loop*v_eta[i][0];
          eta[i][1] += dt2loop*v_eta[i][1];
...
void FixNHMassive::nhc_press_integrate()
{
  int ich,i,pdof;
...
void FixNHMassive::nh_omega_dot()
{
